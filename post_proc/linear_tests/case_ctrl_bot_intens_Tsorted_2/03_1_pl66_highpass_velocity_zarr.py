...
def process_uv_level(level_index: int, data_dir: str, nt: int | None, dt_hours: float = 1.0, T_hours: float = 33.0) -> None:
    """Process u_bc and v_bc successively for one depth level to minimise peak memory."""
    for in_var, out_var in (("u_bc", "u_bc_highpass_pl66"), ("v_bc", "v_bc_highpass_pl66")):
        input_zarr = os.path.join(data_dir, f"{in_var}.zarr")
        output_zarr = os.path.join(data_dir, f"{out_var}.zarr")
        print(f"Highpass filtering {in_var} at depth level {level_index} with PL66...", flush=True)
        process_level(
            level_index=level_index,
            input_zarr=input_zarr,
            input_var=in_var,
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="PL66 highpass-filter baroclinic velocity (u_bc, v_bc) for one depth level using Zarr I/O."
    )
